...
현재 디렉터리 구조는 다음처럼 매핑하라.

| Conceptual Layer            | Current Location                                              | Responsibility                             | Must Not Do                                  |
| --------------------------- | ------------------------------------------------------------- | ------------------------------------------ | -------------------------------------------- |
| Presentation / Delivery     | `src/presentation`, `src/extension.js`                        | 명령 전달, 트리 뷰, 사용자 입력, 결과 표시, host lifecycle | 도메인 규칙 구현, 파일시스템 직접 조작                       |
| Interface Adapters          | `src/presentation`의 입출력 변환기와 `src/infrastructure`의 port 구현 경계 | 외부 형식과 유스케이스 DTO 또는 port 계약 사이의 변환         | 정책 결정, 유스케이스 흐름 소유                           |
| Application / Use Cases     | `src/application`                                             | 유스케이스 조합, 상태 흐름, port 호출, 명시적 결과 반환        | VSCode API 직접 의존, 숨겨진 환경 조회, 구체 adapter 생성   |
| Domain                      | `src/domain`                                                  | 스킬, target, 분석 결과, 상태 전이, 값 객체, 정책         | 외부 API, DB, 파일시스템, 네트워크, 셸, 환경 변수, UI, 로거 의존 |
| Infrastructure / Frameworks | `src/infrastructure`, composition root가 생성하는 host adapter     | 파일시스템, VSCode API, 설정 수신, 외부 도구 연동         | 도메인 정책 또는 유스케이스 결과 결정                        |

개념적 계층을 위해 현재 네 개의 최상위 소스 디렉터리를 기계적으로 늘리지 마라. 하나의 파일이 여러 계층 책임을 섞지 못하게 하고, 새 책임의 소유 계층을 위 표로 판정하라.
...
Input
UseCase
Closed Result: Success | Failure | AdditionalInputRequired
Ports
Domain Policies
```

유스케이스마다 입력 DTO와 출력 DTO를 정의하라. 성공, 실패, 추가 입력 필요 결과를 닫힌 결과 타입 또는 동등한 명시적 계약으로 반환하고, `null`, 예외 문자열, UI 부작용으로 결과 의미를 암묵적으로 전달하지 마라.

유스케이스는 사용자 인터페이스와 인프라 세부사항을 알지 못하게 하라. 유스케이스는 port 인터페이스를 호출하고, 어댑터가 실제 파일시스템, 설정, 로그, 외부 API를 처리하게 하라.

### 2.1 SOLID Rules

- 단일 책임 원칙을 지켜라. 하나의 모듈 또는 타입은 하나의 변경 이유만 갖게 하라.
- 개방-폐쇄 원칙을 지켜라. 새 client, 저장소, 분석기, logger는 핵심 유스케이스 조건문을 늘리지 말고 port 구현 추가로 수용하라.
- 리스코프 치환 원칙을 지켜라. 모든 adapter와 test double은 port의 입력, 출력, 오류 의미, 부작용 계약을 동일하게 지켜라.
- 인터페이스 분리 원칙을 지켜라. 호출자가 사용하지 않는 메서드에 의존하지 않도록 읽기, 쓰기, 분석, 로그 목적별로 작은 port를 정의하라.
- 의존성 역전 원칙을 지켜라. Domain과 Application은 구체 adapter가 아니라 자신들이 소유한 추상화에만 의존하게 하라.
...
```text
Presentation / Delivery -> Interface Adapters -> Application -> Domain
Infrastructure / Frameworks -> Application / Domain Ports
...
- Interface Adapter는 외부 입력을 Application 입력 계약으로 변환하고 Application 출력을 외부 표현으로 변환할 수 있다.
...
- 외부 adapter가 Domain 또는 Application 상태를 직접 변경하지 마라.
...
port는 Application 또는 Domain 경계가 소유하게 하라. 구현체는 Infrastructure에 두라. 구체 adapter 생성과 연결은 `src/extension-composition.js` 같은 composition root에서만 수행하라.
...
3. 코드 기본값과 명시적 유스케이스 입력으로 표현할 수 있는 동작을 외부 설정으로 만들지 마라.
4. 실행 프로세스 중간에 환경 또는 설정 값을 삽입하거나 변경하지 마라.
5. 외부 환경 값은 프로그램 시작 시 최초 1회만 받아들여라.
6. 원본 환경 map, 환경 변수 이름, 설정 파일 형식을 Domain과 Application에 전달하지 마라.
7. 최초 수신 이후에는 외부 환경 값을 검증된 typed `RuntimeContext`로 변환하라.
8. 내부 흐름에서는 명시적 인자, 생성자 인자, 함수 인자, 컨텍스트 객체, 의존성 주입 형태로 전달하라.
9. 암묵적 전역 접근, 전역 설정 상수, static getter, 런타임 중간 재설정, hot reload, 동적 환경 확장, 숨겨진 설정 조회를 금지하라.
...
설정 변경은 실행 중인 유스케이스나 RuntimeContext에 침투시키지 마라. host가 설정 변경을 알리면 현재 application composition을 종료하고 Delivery 경계에서 외부 설정을 다시 한 번 수신하여 새 RuntimeContext와 새 application composition을 생성하라. 기존 인스턴스의 설정을 mutate하거나 유스케이스 내부에서 재조회하지 마라.
...
composition root의 필수 시작 흐름을 다음처럼 유지하라.
...
- singleton, service locator, static getter에서 설정 조회

테스트는 실제 process 환경이나 host 설정을 변경하지 말고 명시적인 typed `RuntimeContext` 또는 `Config` fixture를 주입하라.
...
- 실패한 작업의 reason code와 최소 오류 분류
- 비민감 correlation ID 또는 operation ID
- 사용자 영향이 있는 상태 변화
...
- 요청, operation, component 범위와 보존 기간을 지정하라.
- 기본값은 비활성화하라.
...
- 외부로 전송 가능한 개인정보
- 장기 보존되는 상세 실행 trace
- 원문 request 또는 response payload
- 제한 없는 stdout 또는 stderr
- 전체 prompt 또는 전체 model response
...
- merge 후에도 제품 경계 밖에서 실행되는 임시 trace
...
- 제거할 수 없는 진단이 개발 전용 경계로 격리되고 비활성화되었는가?
...
- 모든 상태 변경은 결정적으로 처리하고 상태 전이 단위 테스트를 작성하라.
- Product Log는 비민감 operation ID, 이전 상태, 다음 상태, reason code를 사용하여 terminal state와 사용자 영향이 있는 transition 또는 failure state를 기록한다.
- Field Debug Log는 제한적으로 transition trace를 기록한다.
- Development Log는 테스트와 로컬 검증에서만 상세 상태를 기록한다.

상태머신은 Domain 또는 Application 유스케이스 규칙 안에 둔다. UI, VSCode API, 파일시스템 어댑터, 외부 client adapter에 상태머신을 종속시키지 마라. 외부 adapter가 상태를 직접 변경하지 못하게 하고, adapter 결과를 이벤트로 변환하여 상태머신에 전달하라.

`isDone`, `shouldRetry`, `isApproved` 같은 boolean flag 조합으로 절차 상태를 추론하거나 제어하지 마라.
...
6. 외부 API, 파일, DB, 네트워크, 명령 실행, 시간은 test double 또는 제어 가능한 interface로 대체하라.
...
- 별도 커밋이 불가능하면 diff와 변경 설명에서 정리 변경과 기능 변경을 명시적으로 구분하라.
- 같은 PR 안에 포함하더라도 커밋 또는 변경 설명을 분리하라.
- 정리 작업이 커지면 별도 작업으로 분리하라.
- 현재 작업과 무관한 대규모 정리를 금지하라.
...
- 외부 I/O가 경계 계층에만 있는가?
- 인터페이스와 구현체가 분리되었는가?
- 모듈 또는 타입이 하나의 변경 이유만 갖는가?
- 새 외부 구현이 핵심 유스케이스 수정 없이 port 구현으로 추가되는가?
- adapter와 test double이 port의 입력, 출력, 오류 의미를 보존하는가?
- port가 호출자 목적별로 작게 분리되었는가?

Domain and Use Cases:

- 유스케이스 입력과 출력이 명확한가?
- 성공, 실패, 추가 입력 필요 결과가 닫힌 결과 타입 또는 동등한 계약으로 표현되는가?
...
- 원본 환경 map이나 외부 설정 형식이 Domain 또는 Application으로 전달되지 않는가?
- 테스트가 실제 process 환경 변경 대신 typed Config fixture를 사용하는가?
...
- Domain에서 HTTP, DB, 네트워크, 셸, UI 직접 접근
- Domain에서 환경 변수 직접 조회
- 유스케이스 내부에서 설정 파일 재로딩
- 유스케이스 내부에서 구체 HTTP client, DB driver, filesystem, network client, process runner 생성 또는 호출
- 전역 mutable 설정 객체
- 전역 설정 상수 또는 static configuration getter
- hidden singleton service locator
- 런타임 중간 process environment 변경
- 실행 중 설정 hot reload 또는 동적 설정 확장
- 암묵적 전역 logger 호출
...
- 실패 상태 없는 절차 흐름
- `isDone`, `shouldRetry`, `isApproved` 같은 boolean flag 조합으로 절차 제어
- Product Log에 민감 정보 기록
- Product Log에 원문 request, response, payload 또는 전체 stack trace 기록
- Field Debug Log를 기본 활성화
- Field Debug Log에 제한 없는 stdout, stderr, 전체 prompt 또는 전체 model response 기록
...
2. 기존 소스 구조, 관련 테스트, composition root를 읽고 변경 범위와 영향 계층을 설명하라.
3. 기능 변경 전에 필요한 Tidy First 작업을 분리하라.
4. 기능 변경 전에 실패하는 테스트를 먼저 작성하라. 작성할 수 없으면 그 이유와 테스트를 막는 경계 설계 문제를 보고하라.
5. 도메인 규칙을 Domain 또는 Application에 배치하라.
6. Domain 또는 Application에 환경 변수, 파일, 네트워크, DB, 셸 접근이 있으면 기능 확장 전에 port와 adapter 경계 뒤로 이동하라.
7. 설정값은 RuntimeContext 또는 명시적 인자로 전달하라.
8. 로그는 Product, Field Debug, Development 중 하나로 분류하라.
9. 복잡한 내부 절차는 상태머신으로 모델링하라.
10. 설정, 로그, 상태 전이 변경에는 각각 독립된 검증 항목을 추가하라.
11. 구현 후 `npm test`와 `npm run build`를 실행하고 결과를 보고하라.
12. 실행하지 않은 검증 항목을 완료로 표시하지 마라. 테스트를 실행하지 못하면 그 이유를 명확히 기록하라.
13. 사용자 변경사항을 되돌리지 마라.
14. 관련 없는 리팩터링을 수행하지 마라.
15. 문서와 코드가 충돌하면 문서를 먼저 갱신하거나 사용자에게 확인하라.
16. 이 문서와 변경 설명은 명령형으로 작성하라. 예외가 필요하면 허용 조건, 승인 주체, 검증 기준을 명시하라.
...
### 13.9 Boundary and Contract Decisions

| 상황                                   | 필수 결정                                                                  |
| ------------------------------------ | ---------------------------------------------------------------------- |
| 유스케이스가 외부 환경 값을 필요로 함                | 시작 시 검증된 typed RuntimeContext를 composition root에서 주입하라.                |
| 외부 API, DB, 파일시스템, 네트워크 또는 셸 호출이 필요함 | Application 또는 Domain이 목적별 port를 소유하고 Infrastructure adapter가 구현하게 하라. |
| Domain 규칙이 외부 정보가 필요함                | Application이 port로 값을 획득하여 Domain에는 검증된 값만 전달하라.                       |
| 유스케이스 결과가 모호함                        | 입력 DTO, 출력 DTO, 성공·실패·추가 입력 필요 결과 계약을 먼저 정의하라.                         |
| 일시적 운영 진단이 필요함                       | 기본 비활성화된 Field Debug Log에 범위, 승인 조건, 보존 기간을 명시하라.                      |
| 재시도와 종료 판단이 함께 필요함                   | 상태, 이벤트, guard, 실패 상태, 종료 상태를 정의하고 전이 테스트를 먼저 작성하라.                    |
| 구조 개선과 기능 추가가 함께 필요함                 | 동작 없는 Tidy First 변경을 분리하고 테스트로 동작 보존을 확인한 뒤 기능을 구현하라.                  |